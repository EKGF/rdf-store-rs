...
use serde_json::json;
use {
    crate::{literal::this::Literal, RDFStoreError},
    iref::IriBuf,
    std::str::FromStr,
};
...
#[test]
fn test_id_url_01() -> Result<(), RDFStoreError> {
    let id_base_iri = IriBuf::from_str("https://whatever.kg/id/")?;
    let literal = Literal::from_iri(&IriBuf::from_str("https://whatever.kg/id/abc")?.as_iri());
...
#[test]
fn test_id_url_02() -> Result<(), RDFStoreError> {
    let id_base_iri = IriBuf::from_str("https://whatever.kg/id/")?;
    let literal = Literal::from_iri(&IriBuf::from_str("https://whatever.kg/id/abc")?.as_iri());
    assert!(literal.is_ok());
    let literal = literal.unwrap();
